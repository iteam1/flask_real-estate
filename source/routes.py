...
def query():
    if request.args:
        ...
        if "area" in args:
            result = Realestate.query.filter_by(area = args.get('area'))
            return realestates_schema.jsonify(result)

        if "max_sqm" in args:
                max_sqm = float(args.get("max_sqm"))
                min_sqm = float(args.get("min_sqm"))
                result = Realestate.query.filter(and_(Realestate.sqm > min_sqm,Realestate.sqm < max_sqm))
                return realestates_schema.jsonify(result)

        if "max_price" in args:
            max_price = float(args.get("max_price"))
            min_price = float(args.get("min_price"))
            result = Realestate.query.filter(and_(Realestate.price > min_price, Realestate.price < max_price))
            return realestates_schema.jsonify(result)

    return "No query string to search"
...
"""
Get the length of the table realesatte
"""
@app.route("/realestate_count",methods = ['GET'])
def count_record():
    length = Realestate.query.count()
    return str(length)
